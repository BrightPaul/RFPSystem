    ...
    if not self.constructionComplete then
        ...
        if Server then
            ...
            if newBuildTime >= timeToComplete then
                ...
                // Give points for building structures Mod: add more if needed
                if self:GetIsBuilt() and not self:isa("Hydra") and not self:isa("Cyst")  and builder and HasMixin(builder, "Scoring") then                
                    if self:isa("BabblerEgg") then builder:AddScore(kBabblerEggPoint)
                    else builder:AddScore(kBuildPointValue) end
                end
